...
- **已落地章节或 capstone**（对应 OpenSpec change 已落地）MAY 包含示例源码（`.go`）、测试文件（`_test.go`）、benchmark 测试以及 `EXERCISES.md` 等学习材料；其 `README.md` MUST 不再保留"待 OpenSpec change 填充"占位。

无论落地与否，仓库整体 MUST 始终保持 `go build ./...` 成功。

#### Scenario: 未落地章节目录内容受严格约束

- **WHEN** 阅读者列出一个尚未落地的章节目录（如 `stage-2-business/08-web-foundations/`）
- **THEN** 该目录 MUST 仅包含 `README.md` 与 `.gitkeep` 两个文件，且 MUST NOT 存在任何后缀为 `.go` 的文件

#### Scenario: 已落地章节目录可包含源码与测试

- **WHEN** 阅读者列出一个已落地的章节目录或 capstone（如 `stage-1-syntax/capstone-1-cli-todo/`）
