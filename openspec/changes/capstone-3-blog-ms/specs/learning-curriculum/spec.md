...
- **已落地 Capstone 3** MUST 提供三个独立博客微服务、HTTP Gateway、gRPC 合同、端到端追踪、韧性策略、容器化配置和自包含冒烟测试。
...
- **WHEN** 阅读者列出一个已落地的章节目录
- **THEN** 该目录 MAY 包含 `.go` 源码、`_test.go` 测试与 `EXERCISES.md`，且其 `README.md` MUST NOT 再包含"待 OpenSpec change 填充"占位语

#### Scenario: Capstone 3 已落地

- **WHEN** 阅读者列出 `stage-3-architecture/capstone-3-blog-ms/`
- **THEN** 目录 MUST 包含三个服务、Gateway、protobuf 合同、端到端测试、Dockerfile、docker-compose、README 和练习材料
