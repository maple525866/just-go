...
无论落地与否，仓库整体 MUST 始终保持 `go build ./...` 成功。第 08 章落地后，`stage-2-business/08-web-foundations/` SHALL 被视为已落地章节，MAY 包含 Web 服务源码、HTTP 测试与练习材料。
...
- **WHEN** 阅读者列出一个已落地的章节目录（如 `stage-2-business/08-web-foundations/`）
