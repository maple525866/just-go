...
无论落地与否，仓库整体 MUST 始终保持 `go build ./...` 成功。第 10 章落地后，`stage-2-business/10-cache-and-mq/` SHALL 被视为已落地章节，MAY 包含缓存与消息源码、测试与练习材料。
...
- **WHEN** 阅读者列出一个已落地的章节目录（如 `stage-2-business/10-cache-and-mq/`）
