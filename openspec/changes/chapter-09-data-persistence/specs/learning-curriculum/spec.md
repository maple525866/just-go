...
无论落地与否，仓库整体 MUST 始终保持 `go build ./...` 成功。第 09 章落地后，`stage-2-business/09-data-persistence/` SHALL 被视为已落地章节，MAY 包含数据持久化源码、数据库测试、迁移 SQL 与练习材料。

#### Scenario: 未落地章节目录内容受严格约束

- **WHEN** 阅读者列出一个尚未落地的章节目录
- **THEN** 该目录 MUST 仅包含 `README.md` 与 `.gitkeep` 两个文件，且 MUST NOT 存在任何后缀为 `.go` 的文件

#### Scenario: 已落地章节目录可包含源码与测试

- **WHEN** 阅读者列出一个已落地的章节目录（如 `stage-2-business/09-data-persistence/`）
- **THEN** 该目录 MAY 包含 `.go` 源码、`_test.go` 测试、迁移 SQL 与 `EXERCISES.md`，且其 `README.md` MUST NOT 再包含“待 OpenSpec change 填充”占位语
