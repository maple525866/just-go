...
在本次提案落地后，每一个章节目录与 capstone 目录 MUST 仅包含两个文件：一个使用统一模板生成的 `README.md`，以及一个用于保证 Git 跟踪空目录的 `.gitkeep` 文件。任何 `.go` 源码文件 MUST 不存在于章节目录内（章节代码由后续独立的 OpenSpec change 落地）。

#### Scenario: 章节目录内容受严格约束

- **WHEN** 阅读者列出任意章节目录（如 `stage-1-syntax/01-hello-go/`）
- **THEN** 该目录 MUST 仅包含 `README.md` 与 `.gitkeep` 两个文件，且 MUST NOT 存在任何后缀为 `.go` 的文件

#### Scenario: 仓库整体可通过 go build

- **WHEN** 在仓库根目录执行 `go build ./...`
- **THEN** 命令 MUST 以退出码 0 成功返回（因不存在任何 `.go` 文件，无包可编译）
